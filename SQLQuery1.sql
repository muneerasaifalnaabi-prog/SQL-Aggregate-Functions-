...
--1. Count employees with non-null salary
SELECT COUNT(Salary) AS EmployeesWithSalary
FROM Employee1;

--2. Departments including those with NULL salaries
SELECT DNUM,
       COUNT(Salary) AS EmployeesWithSalary
FROM Employee1
GROUP BY DNUM;

--3. Average salary ignoring NULL values
SELECT DNUM,
       AVG(Salary) AS AvgSalary
FROM Employee1
GROUP BY DNUM;

--4. Departments where all salaries are NULL
SELECT DNUM
FROM Employee1
GROUP BY DNUM
